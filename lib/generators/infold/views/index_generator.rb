...
module Infold
  module Views
    class IndexGenerator < Rails::Generators::NamedBase
      ...
      def index_file
        template "views/index.haml", Rails.root.join("infold", "#{name.underscore.singularize}_view_index.html.haml"), force: true
      end

      def index_row_file
        template "views/_index_row.haml", Rails.root.join("infold", "#{name.underscore.singularize}_view_index_row.html.haml"), force: true
      end
    end
  end
end